...
class QuickMailingListSignup(FormView):
    ...
    def form_valid(self, form):
        ...
        if captcha is None and not self.request.user.is_authenticated:
            ...
            return render(
                self.request, self.template_name, {"form": next_form}
            )
        ...
        if not user:
            try:
                ...
                user.email = email
                logger.info(
                    f"QuickMailingListSignup: Created new user {user.email}"
                )
                user.save()
        ...
        try:
            ...
            logger.info(f"Failed to find mailing_list_token={mailinglist}")
        ...
        if mailing_list_obj.linked_article:
            ...
            url = (
                reverse_lazy("topicblog:view_item_by_slug", kwargs=kwargs)
    ...
    def form_invalid(self, form):
        ...
        if origin_url:
            messages.add_message(
                self.request, messages.ERROR, form.errors.as_text()
            )
...
class PetitionView(AssoView):
    def get_context_data(self, **kwargs):
        ...
        context["page"] = {
            "hero_image": "asso_tn/traffic-1600.jpg",
...
class MailingListListView(
    ...
    def get_context_data(self, **kwargs):
        ...
        context["mailing_lists"] = [
            (list, user_subscribe_count(list))
            for list in lists
            if not list.is_petition
        ]
        context["petitions_lists"] = [
            (list, subscriber_count(list))
            for list in lists
            if list.is_petition
        ]
        return context
...
class UserStatusView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        ).order_by("mailing_list_name")
        user = self.request.user
        object_list_with_state = [
            (
                mailing_list,
                user_current_state(user, mailing_list).event_type,
            )
...
class PressSubscriptionManagementView(TemplateView):
    ...
    def get_context_data(self, **kwargs) -> dict:
        ...
        if email and send_record_id:
            ...
            context["press_subscription_list"] = (
                MailingList.objects.filter(mailing_list_type="PRESS")
                .order_by("-id")
                .annotate(  # noqa
                    is_subbed=Subquery(
                        MailingListEvent.objects.filter(
                            mailing_list=OuterRef("id"), user=context["user"]
                        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        ).exclude(id__in=ml_id_to_subscribe_to)
