...
class DonationForm(forms.Form):
    ...
    last_name = forms.CharField(
        label="Nom", widget=forms.TextInput(attrs={"placeholder": "Nom*"})
    )
    ...
    address = forms.CharField(
        label="Adresse",
        widget=forms.TextInput(attrs={"placeholder": "Adresse postale*"}),
    )

    more_address = forms.CharField(
    ...
    city = forms.CharField(
        label="Ville", widget=forms.TextInput(attrs={"placeholder": "Ville*"})
    )
    ...
    data_collect = forms.BooleanField(
        label="J'accepte que mes données soient \
...
class AmountForm(forms.Form):
    ...
    donation_type = forms.ChoiceField(
        label="", choices=sub_or_donate, widget=forms.RadioSelect
    )

    # Initialized in views.py by get_amount_choices()
    payment_amount = forms.ChoiceField(
        label="", choices=[], widget=forms.RadioSelect
    )

    # initialized in views.py by get_subscription_amounts()
    subscription_amount = forms.ChoiceField(
        label="", choices=[], widget=forms.RadioSelect
    )

    free_amount = forms.IntegerField(
        label="Montant libre",
        min_value=1,
        required=False,
        widget=forms.TextInput(attrs={"placeholder": "Montant libre"}),
    )  # noqa
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Field(
                "donation_type", template="stripe_app/inline_radio_custom.html"
            ),
            Field(
                "subscription_amount",
                template="stripe_app/inline_radio_custom.html",
                id="subscription_amount_rb",
                style="display: none",
            ),
            Field(
                "payment_amount",
                template="stripe_app/inline_radio_custom.html",
                id="payment_amount_rb",
                style="display: none",
            ),
...
class QuickDonationForm(forms.Form):
    ...
    amount = forms.IntegerField(
        ...
        min_value=1,
        widget=forms.TextInput(
            attrs={"placeholder": "Montant en Euros*", "readonly": "readonly"}
        ),
        required=True,
    )
    ...
    donation_type = forms.CharField(
        max_length=20, initial="payment", label="", required=False
    )
    payment_amount = forms.CharField(
        max_length=20, initial="", label="", required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            Field(
                "payment_amount", id="payment_amount", style="display: none"
            ),
