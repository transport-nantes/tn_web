...
class GiletReserveView(FormView):
    ...
    def form_valid(self, form):
        ...
        user.profile.save()
        gilet = ClickableCollectable.objects.get(
            collectable_token="gilet-transport-nantes"
        )
        click_and_collect = ClickAndCollect.objects.create(
            user=user, collectable=gilet
        )
        click_and_collect.save()
        # return super(GiletReserveView, self).form_valid(form)
        return render(
            self.request,
            "clickcollect/reserved.html",
            {
                "hero": True,
