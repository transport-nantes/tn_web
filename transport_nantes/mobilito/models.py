...
class MobilitoSession(models.Model):
    ...
    def create_event(self, event_type):
        ...
        logger.info(
            f"{self.user.user.email} created an event of type {event_type}"
        )
    ...
    def generate_session_sha1(self):
        ...
        return hashlib.sha1(
            (str(time.monotonic_ns()) + "|" + str(self.user.user.id)).encode(
                "utf-8"
            )
        ).hexdigest()
...
class Event(models.Model):
    ...
    mobilito_session = models.ForeignKey(
        MobilitoSession, on_delete=models.PROTECT
    )
    ...
    def __str__(self):
        return (
            f"{self.mobilito_session.user.user.email} - {self.timestamp}"
            f" - {self.event_type}"
        )
...
class InappropriateFlag(models.Model):
    ...
    def __str__(self):
        return (
            f"Report on {self.session.user.user.email} - "
