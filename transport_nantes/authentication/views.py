        ...
        if form.is_valid():
            ...
            if password1:
                ...
                if len(existing_users) == 1:
                    ...
                    user = auth.authenticate(username=existing_user,
                                             password=password1)
                ...
                # If user doesn't exist and give password, create account
            ...
            # If no password given
                ...
                if len(existing_users) == 1:
                    ...
                    if existing_user.profile.authenticates_by_mail:
                        ...
                        return render(
                            request,
                            'authentication/account_activation_sent.html',
                            {'is_new': False})
                # If user doesn't exist, create account and
                ...
                if len(existing_users) == 0:
                    ...
                    send_activation(request, user, True, remember_me)
                    return render(
                        request, 'authentication/account_activation_sent.html',
                        {'is_new': True})
        else:
            # # Form is not valid.
            context = {}
...
def activate(request, token, remember_me, base_template=None):
    ...
    try:
        ...
        return render(
            request, 'authentication/account_activation_invalid.html')
...
def profile(request):
    if request.method == 'POST':
        ...
        if u_form.is_valid() and p_form.is_valid():
            u_form.save()
            p_form.save()
            # messages.success(request, f'Your Profile has \
