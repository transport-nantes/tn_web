...
# what you were trying to do.  For example, you call
# self.client.get/post and assign to a variable called request.  But
# that function returns a response.  Then you access fields that make
# sense for requests (or that don't exist at all).
#
# So I'm leaving in my branch what you did in your branch, but there's
# an element of wtf about it.  I'd think you'd write a test, get it
# working, iterate.  Instead, you wrote a ton of tests, many of which
# seem not to work.
#
# That said, I know I've chaged some stuff, and that's invalidated
# some of your tests.
#
# Some suggestions:
#   * avoid magic numbers (e.g., use k_six_months_in_seconds rather than 15552000)
#   * use IANA reserved names for testing (e.g., example.com) rather than making
#     things up that could, actually, exist, or change existance).  Cf RFC 2606
#     and RFC 6761.
...
#         try:
#             test_user = User.objects.get(email="test_user@example.com")
...
#         # Add existing user in database
#         User.objects.create_user(username="test_user", email="test1@example.com")
#         # Test addition of user
#         try:
...
#         # Add existing user in database
#         User.objects.create_user(username="test_user1", email="test@example.com")
#         User.objects.create_user(username="test_user2", email="test@example.com")
#         # Test addition of user and that number of users superior to 1
#         try:
#             test_user_count = User.objects.count()
#             test_user = User.objects.filter(email="test@example.com")
#             existing_test_user = True
#         except:
#             existing_test_user = False
...
#         # Add existing user in database
#         User.objects.create_user(username="test_user", email="test1@example.com", password="secret_password")
#         # Test addition of user
#         try:
#             test_user = User.objects.get(email="test1@example.com")
...
#         self.assertEqual(max_age, self.k_six_months_in_seconds)
...
#         User.objects.create_user(username="test_user", email=email, password="secret_password")
#         # Test addition of user
#         try:
