...
class MapView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            raise Http404(
                f"{city} ou l'observatoire {observatory_name}\
                n'existe(nt) pas"
            )
        ...
        unique_layers = map_content_rows.values(
            "map_layer__layer_name"
        ...
        styles = [
            {"color": "#000000", "dashArray": "1 10"},
            {"color": "#9BBF85", "weight": 7},  # 006164, #9BBF85
            {"color": "#B3589A", "weight": 7},
        ]  # B3589A , #DB4325
        ...
        context["social"][
            "og_description"
        ] = "Suivez l'avancement du vélo à Nantes Métropole"
        context["social"][
            "og_image"
        ] = "asso_tn/20210228-210131-9011_velorution-psm-sm.jpg"
...
class DownloadGeoJSONView(View):
    def get(self, request, **kwargs):
        ...
        filename = (
            "MOBILITAINS_"
            + str(last_layer.map_layer.layer_name).replace(" ", "_")
            + "_"
            + str(last_layer.timestamp)
            .replace(":", "-")
            .replace(" ", "_")[:19]
            + ".geojson"
        )
